...
func uniquePositions(m map[rune][]pos, antip map[rune][]pos) int {
	var pm []pos
	for key := range antip {
		for i := range antip[key] {
			p := antip[key][i]
	...
	for key := range m {
		for i := range m[key] {
			...
			if !slices.Contains(pm, p) {
				pm = append(pm, p)
			//} else {
			//	log.Printf("DEBUG: duplicate m: %v\n", p)
			}
		}
	}
...
func main() {
	...
	antiSum := uniquePositions(m, antip)
